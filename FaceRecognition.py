...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        # print(confidence)
        if confidence < 80 and a>0.999:
            text = name[str(idnum)] +  'ok~'                      # 如果信心指數小於 80，取得對應的名字
            print(text)
        if confidence < 80 and b>0.001:
            text = name[str(idnum)] +  'no mask!!'                # 如果信心指數小於 80，取得對應的名字
            print(text)
        if confidence >= 80 and a>0.9991:
            text = 'Who are you?' +  'ok~'                        # 如果信心指數小於 80，取得對應的名字
            print(text)
        else:
            text = 'Who are you?' +  'no mask!!'                  # 不然名字就是 ???
